...
Pin a version tag such as `v1.7.1` for production deployments. Use `latest` only for short trials, because pulling or restarting later may upgrade unexpectedly.
...
services:
  browseforge:
    image: ghcr.io/nczz/browseforge:v1.7.1
...
```bash
docker pull ghcr.io/nczz/browseforge:v1.7.1
