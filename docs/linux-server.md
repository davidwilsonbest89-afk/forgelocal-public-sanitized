...
docker run -d --name browseforge \
  ...
  ghcr.io/nczz/browseforge:v2.1.11
```

Pin a version tag such as `v2.1.11` for production deployments. Use `latest` only for short trials, because pulling or restarting later may upgrade unexpectedly.
...
services:
  browseforge:
    image: ghcr.io/nczz/browseforge:v2.1.11
    ports:
      - "19280:19280"
      - "6901:6901"
    volumes:
...
```bash
docker pull ghcr.io/nczz/browseforge:v2.1.11
