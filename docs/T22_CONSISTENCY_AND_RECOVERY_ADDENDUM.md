...
T22 ne revendique pas une transaction ACID unique entre `profile.json` et SQLite. Chaque mutation persistante de Profile écrit d’abord un **marqueur pending durable**, owner-only, adjacent au profil. Ce marqueur porte un identifiant d’opération non secret et imprévisible, une action, une date et un digest déterministe de la représentation Profile redacted à historiser. Il n’est jamais projeté par l’API ni copié dans un snapshot History.

Après l’écriture Profile, History tente une capture dans sa transaction SQLite en y enregistrant le même identifiant d’opération. La capture réussie est suivie de la suppression conditionnelle du marqueur portant le même identifiant. La séquence complète est sérialisée par profil au niveau du handler. Si History échoue, le profil demeure marqué pending et la réponse est un refus explicite ; l’état est alors récupérable, jamais silencieusement déclaré historisé.
