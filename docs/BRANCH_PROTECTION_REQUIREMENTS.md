...
|---|---|---|
| `.github/CODEOWNERS`, `.github/production-release-policy.json` et `.github/workflows/` | Sécurité, Release et relectrice indépendante | Protéger la politique d’ownership, la déclaration des approbatrices de release et les contrôles d’automatisation. |
...
Le job `release` du workflow est maintenant rattaché à l’environnement `production-release`, après `provenance → verify → build`. La déclaration versionnée [`.github/production-release-policy.json`](../.github/production-release-policy.json) exige deux approbatrices : **Release** `@davidwilsonbest89-afk` et **indépendante** `@hajarbenmlih91-cloud`. L’administrateur doit créer cet environnement dans **Settings → Environments**, ajouter ces deux comptes comme reviewers requis, cocher l’interdiction de self-review, désactiver le bypass administrateur si cette option est proposée et limiter les références de déploiement aux tags `v*`. Les secrets de signature et de publication doivent être ajoutés uniquement dans cet environnement, jamais dans le code, les secrets globaux du dépôt ou les logs.
...
Pour l’environnement, il doit déclencher une release de test non publique ou une exécution contrôlée, vérifier l’arrêt du job `release` dans l’attente des deux reviewers `@davidwilsonbest89-afk` et `@hajarbenmlih91-cloud`, confirmer que l’auteur ne peut pas s’auto-approuver puis vérifier que les secrets ne sont disponibles qu’après approbation. Aucun secret ne doit être affiché dans la preuve.
...
| Environnement `production-release` | Déclaré dans le workflow et la politique versionnée ; configuration distante **PENDING** | Reviewers Release et indépendante, self-review interdite, bypass désactivé et secrets limités à l’environnement. |
