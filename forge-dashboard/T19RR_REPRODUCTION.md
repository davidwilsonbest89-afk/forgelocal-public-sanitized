...
`tests/automation-t15.spec.ts` crée désormais sa propre fixture HTML dans un répertoire temporaire unique via `beforeAll`, utilise l’URL `file://` correspondante, puis supprime le répertoire via `afterAll`. Aucun fichier sous `/tmp/t15-fixtures` ne doit être préparé manuellement.

Les scénarios T14 et T15 utilisent `FORGELOCAL_DASHBOARD_URL` pour le navigateur, et construisent les en-têtes `Origin` et `Referer` à partir de cette URL. Ils n’imposent donc ni `localhost`, ni le port `3000`.
