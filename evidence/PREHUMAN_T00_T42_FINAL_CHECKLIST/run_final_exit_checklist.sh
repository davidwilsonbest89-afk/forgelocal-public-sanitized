...
mkdir -p "$out"
log="$out/PREHUMAN_FINAL_EXIT_CHECKLIST_RAW.log"
: > "$log"
run() {
  label=$1; shift
...
run git_head git rev-parse HEAD
run git_fsck git fsck --full
run git_lfs_fsck git lfs fsck
run go_version go version
run git_version git --version
run git_lfs_version git lfs version
...
run golangci_lint golangci-lint run
run govulncheck govulncheck ./...
run osv_scanner osv-scanner scan source -r .
run trivy_fs trivy fs --scanners vuln,secret,misconfig .
run syft_cyclonedx bash -c 'syft dir:"$1" -o cyclonedx-json="$2"' bash "$repo" "$out/PREHUMAN_FINAL_SBOM.cdx.json"
run syft_spdx bash -c 'syft dir:"$1" -o spdx-json="$2"' bash "$repo" "$out/PREHUMAN_FINAL_SBOM.spdx.json"
run gitleaks_inventory bash -c 'git diff --name-status "t00-t27-complete-20260820..HEAD"' bash
run gitleaks_cumulative bash -c 'git diff --binary "t00-t27-complete-20260820..HEAD" | gitleaks detect --pipe --no-banner --redact --report-format json --report-path "$1/PREHUMAN_FINAL_GITLEAKS.json"' bash "$out"
base=$(mktemp -d)
...
run gosec_baseline bash -c 'cd "$1" && gosec -fmt=json -out="$2/PREHUMAN_FINAL_GOSEC_BASELINE.json" ./...' bash "$base" "$out"
run gosec_head bash -c 'cd "$1" && gosec -fmt=json -out="$2/PREHUMAN_FINAL_GOSEC_HEAD.json" ./...' bash "$repo" "$out"
run gosec_normalized python3 /home/ubuntu/normalize_gosec_comparison.py "$out"
rm -rf "$base"
...
if [ -f forge-dashboard/package.json ]; then
  ...
  if find . -path './node_modules' -prune -o -type f \( -name '*.spec.ts' -o -name '*.spec.tsx' -o -name '*.test.ts' -o -name '*.test.tsx' \) -print -quit | grep -q .; then
    run dashboard_playwright pnpm exec playwright test --workers=1
  else
    printf 'started_utc=%s\ncwd=%s\nhead=%s\ncommand=fixture discovery\n--- raw_output ---\nno applicable Playwright test files found\nexit_code=0\ncompleted_utc=%s\ncheck=dashboard_playwright\n---\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$PWD" "$(git -C "$repo" rev-parse HEAD 2>&1)" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$log"
  fi
fi
cd "$repo"
