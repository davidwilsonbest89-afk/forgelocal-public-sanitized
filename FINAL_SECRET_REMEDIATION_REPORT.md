...
Le workflow `.github/workflows/ci.yml` contient maintenant un job déterministe, un job `security-scans` et un job `browser-runtime-qualified` déclenchable explicitement sur un runner self-hosted portant le label `forgelocal-chromium`. Les contrôles CI ajoutés couvrent Gitleaks source et historique pertinent, Trivy filesystem secrets, Syft source et image, Grype source et image, build de l’image runtime durcie, inspection hardening/layers, cycle anti-fuite, ShellCheck et test authentifié synthétique. Le job déterministe ne lance plus implicitement Chromium : il vérifie que `FORGELOCAL_VALIDATE_CHROMIUM_RUNTIME` n’est pas activé et signale la validation navigateur différée. Le job navigateur exige `FORGELOCAL_VALIDATE_CHROMIUM_RUNTIME=1`, vérifie les binaires et la version Chromium, puis exécute le test AC-BACK-01 ciblé. Le job de release n’a pas été modifié et aucun push d’image n’est effectué par ces gates.

Le workflow reste soumis à la politique de provenance existante, validée localement par `node scripts/check-ci-provenance-workflow.mjs`. Les contrôles frontend ont été exécutés dans `forge-dashboard` avec `pnpm install --frozen-lockfile`, `pnpm run check` et `pnpm run build`, chacun en succès. Une première tentative à la racine a été classée comme commande invalide car le lockfile et les scripts résident dans `forge-dashboard`; elle n’est pas utilisée comme résultat produit. Les vulnérabilités et matches scanners ne sont pas ignorés globalement. Les étapes Grype `report` conservent les rapports comme artefacts et peuvent être `continue-on-error` uniquement pour permettre la génération du rapport ; elles sont immédiatement suivies d’une `Grype policy gate` bloquante. Cette policy échoue pour tout Critical/High non approuvé par une exception exacte comportant owner, justification et expiration. Sur le rapport image réel, elle retourne exit code 1 avec 43 Critical/High ouverts et 0 exception approuvée. La gate Trivy secrets échoue si un secret est présent dans l’image. La définition locale est `CI_GATES_DEFINED_LOCAL_VERIFICATION_PASS`. Des exécutions GitHub Actions distantes réelles ont été observées : la première run `33033705812` a échoué sur la référence inexistante `aquasec/trivy:0.74.2`, puis le workflow a été corrigé vers `0.74.0`. La dernière run ayant exécuté les changements CI est `33083605859`, sur le HEAD `0886b777eb57775e8a53f16dc8f4527ccc27266a` : elle est terminée en échec global, mais le job déterministe `test` est désormais en succès, y compris Go test, race/vet, ShellCheck, authentification synthétique, UI i18n, pnpm install/check/build et provenance. Le job sécurité échoue sur les 43 Critical/High non approuvées, ce qui est le comportement bloquant attendu. Le job `Qualified Chromium runtime validation` est `skipped` sur une exécution pull request ; il reste obligatoire avant release et doit être déclenché sur un runner self-hosted qualifié avec `workflow_dispatch`. Les runs antérieures `33069090849`, `33070413940` et `33071664971` sont conservées pour la traçabilité historique ; leurs échecs Chromium hébergés restent classés `BLOCKED_ENVIRONMENT_REQUIRED` et ne sont pas présentés comme des succès. Le statut reste `CI_REMOTE_EXECUTION_OBSERVED=TRUE`, `CI_REMOTE_EXECUTION_PASS=FALSE`, `CI_REMOTE_EXECUTION_NOT_CONFIRMED=FALSE`, `CI_CURRENT_HEAD_CI_RUN=33083605859`, `CI_CURRENT_HEAD_CI_CONCLUSION=FAILURE`, `CI_REMOTE_EXECUTION_CONCLUSION=FAILURE_GRYPE_GATE_WITH_DETERMINISTIC_JOB_PASS_BROWSER_NOT_EXECUTED`, `BRANCH_PROTECTION_ENFORCEMENT=NOT_ACTIVE`, `SECURITY_JOB_REQUIRED_FOR_PR=NOT_VERIFIED` et `INDEPENDENT_REVIEW_PENDING`. Une re-vérification GitHub CLI antérieure avait retourné HTTP 401 `Bad credentials`, puis l’accès a été rétabli : `CI_REMOTE_FINAL_RECHECK=PASS`. La PR et la protection de branche ont été vérifiées après réauthentification ; la protection reste inactive. Les compléments de pasted_content_13 sont désormais publiés sur la branche corrective ; la CI du commit package est observée en échec sur la policy Grype et le timeout Chromium distant. Aucun changement n’a été effectué sur `main` et aucune image n’a été poussée.
...
CI_REMOTE_EXECUTION_CONCLUSION=FAILURE_GRYPE_GATE_WITH_DETERMINISTIC_JOB_PASS_BROWSER_NOT_EXECUTED
CI_REMOTE_FINAL_RECHECK=PASS
CI_CURRENT_PASTED13_HEAD=0886b777eb57775e8a53f16dc8f4527ccc27266a
CI_REMOTE_RUN_ID=33083605859
CI_REMOTE_RUN_URL=https://github.com/davidwilsonbest89-afk/forgelocal-public-sanitized/actions/runs/33083605859
CI_REMOTE_SECURITY_JOB_ID=98557142563
CI_REMOTE_SECURITY_JOB_CONCLUSION=FAILURE_GRYPE_POLICY_43_UNAPPROVED_CRITICAL_HIGH
CI_REMOTE_TEST_JOB_ID=98557142880
CI_REMOTE_TEST_JOB_CONCLUSION=SUCCESS_DETERMINISTIC_TEST_JOB
CI_REMOTE_BROWSER_JOB_ID=98557144002
CI_REMOTE_BROWSER_JOB_CONCLUSION=SKIPPED_QUALIFIED_RUNNER_REQUIRED
