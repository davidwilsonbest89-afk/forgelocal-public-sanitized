...
func TestFindBinaryLocatesBrowseForgeChromiumArtifactLayouts(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			binaryRel: filepath.Join("browseforge-runtime-chromium-v0.1.4-alpha.0-macos-x64", "Chromium.app", "Contents", "MacOS", "Chromium"),
		},
		{
			name:      "extracted artifact root with linux executable",
			binaryRel: filepath.Join("browseforge-runtime-chromium-v0.1.4-alpha.0-linux-x64", "chrome"),
		},
	}
...
func TestBrowseForgeChromiumDownloadURLForLinuxArm64(t *testing.T) {
	...
	wantFilename := "browseforge-runtime-chromium-v0.1.4-alpha.0-linux-arm64.zip"
	if gotFilename != wantFilename {
		t.Fatalf("filename = %q, want %q", gotFilename, wantFilename)
	}

	wantURL := "https://runtime.example/releases/v0.1.4-alpha.0/" + wantFilename
	if gotURL != wantURL {
